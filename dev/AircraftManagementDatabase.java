 ...
 * The index of each dev.ManagementRecord in the array is its "management code" ("mCode"), and the mCode of any particular visiting aircraft's dev.ManagementRecord must remain fixed once it is allocated.
...
// Todo : change the method to apply on arraylists.
  public List<Integer> getWithStatus(int statusCode){
    ...
    for (int i = 0; i < maxMRs; i++) {
      if(MRs[i].getStatus() == statusCode) list.add(i);
    }
    return list;
  }
