...
/**
 * Contains an array of dev.PassengerDetails objects - one per passenger on a flight.
 * Incoming flights supply their passenger list in their flight descriptor, and the dev.ManagementRecord for the flight extracts the dev.PassengerList and holds it separately.
 * Outbound flights have PassengerLists built from passenger details supplied by the gate consoles, and the list is uploaded to the aircraft as it departs in a newly built dev.FlightDescriptor.
 * @stereotype entity
...
public class PassengerList {
  /**
 * The array of dev.PassengerDetails objects.
 * @byValue
 * @clientCardinality 1
 * @directed true
 * @label contains
 ...
 */
  ...
  public List<String> getPassengerList() {
    return passengerList.stream().map(PassengerDetails::getName).collect(Collectors.toList());
  }
