 ...
 * An instance of dev.AircraftManagementDatabase holds a collection of ManagementRecords, and sends the ManagementRecords messages to control/fetch their status.
 * @stereotype entity
...
/** The status code for this dev.ManagementRecord.*/
  ...
  public void setStatus(int newStatus){
    ...
    // the newStatus is an appropriate transition as detailed in the MRState Diagram.
      switch (status) {
        ...
        case 0:
          if (newStatus == IN_TRANSIT || newStatus == WANTING_TO_LAND) { // newStatus == 2 && dev.Itinerary.getTo() == "Stirling";
            status = newStatus;
          }
          else {
...
/** Sets up the MR with details of newly detected flight
  ...
  * @preconditions Status is FREE*/
  public void radarDetect(FlightDescriptor fd){
    ...
    // Would probably be better served as a try/catch, yes, but it's not a big deal
    // Better safe than sorry; the flight information needs to be cleared if the status change fails
    if (!itinerary.getNext().equalsIgnoreCase("STIRLING")) {
      setStatus(IN_TRANSIT);
    }
    else if (itinerary.getNext().equalsIgnoreCase("STIRLING")) {
...
/** dev.GOC has allocated the given gate for unloading passengers.
  *
...
/** The given passenger is boarding this aircraft.
  ...
  * @preconditions Status is READY_PASSENGERS*/
  public void addPassenger(PassengerDetails details){
    ...
    // The actual work of this method is delegated to dev.PassengerList's 'addPassenger' method!
    passengerList.addPassenger(details, status);
  }

  /** Return the entire current dev.PassengerList.
  ...
  /** Return the aircraft's dev.Itinerary.
  *  Will return NULL if the gate status is FREE
  *
  */
